...
// Save query data sent by user.
app.post("/savequerydata", function (req, res) {
    console.log("node projID: ",req.body.projID);
    var projID = req.body.projID;
    var feedback = [];
    //console.log("node mp:", req.body.mp);

    var tweets = JSON.parse(req.body.mp);
    // Debugging
    /* 
    console.log("tweets00",tweets['0'][0]);
    console.log("tweets01",tweets['0'][1]);
    console.log("tweets00 len:", tweets['0'].length);
    */
    
    // For each unique key (entry), save that entries first (TWEET_ID) and second (TWEET_TEXT) data point to firebase.
    Object.keys(tweets).forEach(function(key) {
        //console.log("ID:",tweets[key][0], "| Text:",tweets[key][1]);
        db.collection("projects").doc(projID).collection("tweets").doc(tweets[key][0]).set({
            id: tweets[key][0],
            text: tweets[key][1]
        })
        .catch((error) => {
            console.error("Error saving tweet: ", error);
            feedback.push("Error saving tweet: ", error);
        })
      })
    // For .done feedback.
    res.send(feedback);
...
// API call to load saved tweet data.
app.post("/loadtweetdata", function (req, res) {
    // Dict for saving loaded results to send back clientside.
    var result = {}

    // Project ID for accessing Firebase
    var projID = req.body.key;

    // Grab saved data based on currenlty selected project (projID).
    db.collection("projects").doc(projID).collection("tweets").get().then(function (tweets) {
        tweets.forEach(doc => {
            // Save tweet ID and tweet text to dict.
            result[doc.id] = doc.data().text;
        })
        // Why does this have to be inside (async?).
        // Send key value pairs back to client side.
        res.send(result);
    })
    //console.log(result);
    //res.send(result);
